...
std::string variant_error_to_string(const VariantType &error) {
  return std::visit([](const auto &err) { return std::string(err.what()); },
                    error);
}

xgrammar_error_kind
error_kind_from_serialization(const xgrammar::SerializationError &error) {
  ...
  if (std::holds_alternative<xgrammar::InvalidJSONError>(error))
    return XGRAMMAR_ERROR_INVALID_JSON;
  return XGRAMMAR_ERROR_UNKNOWN;
}

xgrammar_error_kind
error_kind_from_structural_tag(const xgrammar::StructuralTagError &error) {
  if (std::holds_alternative<xgrammar::InvalidJSONError>(error))
    return XGRAMMAR_ERROR_INVALID_JSON;
...
std::vector<std::string> to_string_vector(const char *const *strings,
                                          int32_t count) {
...
xgrammar_grammar *xgrammar_grammar_create_from_ebnf(const char *ebnf,
  ...
  return new xgrammar_grammar{xgrammar::Grammar::FromEBNF(
      std::string(ebnf), std::string(root_rule ? root_rule : "root"))};
}

xgrammar_grammar *xgrammar_grammar_create_from_regex(const char *regex) {
  return new xgrammar_grammar{
      xgrammar::Grammar::FromRegex(std::string(regex), false)};
}
...
xgrammar_grammar *xgrammar_grammar_create_from_json_schema(
  ...
  if (has_separators) {
    separators_opt =
        std::make_pair(std::string(sep_item), std::string(sep_line));
  }
  ...
  return new xgrammar_grammar{xgrammar::Grammar::FromJSONSchema(
      std::string(schema), any_whitespace, indent_opt, separators_opt,
      strict_mode, max_ws_opt, print_converted_ebnf)};
}
...
xgrammar_grammar *xgrammar_grammar_create_from_structural_tag(
  ...
  if (std::holds_alternative<xgrammar::Grammar>(result)) {
    if (out_error_kind)
      *out_error_kind = XGRAMMAR_ERROR_NONE;
    return new xgrammar_grammar{std::get<xgrammar::Grammar>(result)};
  }

  const auto &error = std::get<xgrammar::StructuralTagError>(result);
  if (out_error)
    *out_error = copy_string(variant_error_to_string(error));
  if (out_error_kind)
    *out_error_kind = error_kind_from_structural_tag(error);
  return nullptr;
}
...
xgrammar_grammar *xgrammar_grammar_create_from_serialized_json(
  ...
  if (out_error)
    *out_error = copy_string(variant_error_to_string(error));
  if (out_error_kind)
    *out_error_kind = error_kind_from_serialization(error);
  return nullptr;
}
...
xgrammar_compiled_grammar_create_from_serialized_json(
  ...
  auto result = xgrammar::CompiledGrammar::DeserializeJSON(std::string(json),
                                                           tokenizer_info->obj);
  if (std::holds_alternative<xgrammar::CompiledGrammar>(result)) {
    if (out_error_kind)
      *out_error_kind = XGRAMMAR_ERROR_NONE;
    return new xgrammar_compiled_grammar{
...
xgrammar_compiler_create(const xgrammar_tokenizer_info *tokenizer_info,
  ...
  return new xgrammar_grammar_compiler{xgrammar::GrammarCompiler(
      tokenizer_info->obj, max_threads, cache_enabled, max_memory_bytes)};
}
...
xgrammar_compiler_compile_grammar(xgrammar_grammar_compiler *compiler,
  ...
  return new xgrammar_compiled_grammar{
      compiler->obj.CompileGrammar(grammar->obj)};
}
...
xgrammar_compiled_grammar *xgrammar_compiler_compile_json_schema(
  ...
  return new xgrammar_compiled_grammar{compiler->obj.CompileJSONSchema(
      std::string(schema), any_whitespace, indent_opt, separators_opt,
      strict_mode, max_ws_opt)};
}

xgrammar_compiled_grammar *
xgrammar_compiler_compile_builtin_json(xgrammar_grammar_compiler *compiler) {
  return new xgrammar_compiled_grammar{
      compiler->obj.CompileBuiltinJSONGrammar()};
}

int64_t
xgrammar_compiler_cache_size(const xgrammar_grammar_compiler *compiler) {
...
xgrammar_grammar_matcher *xgrammar_matcher_create(
  ...
  if (has_override_stop_tokens) {
    if (override_stop_tokens && override_stop_token_count > 0) {
      override_opt.emplace(override_stop_tokens,
                           override_stop_tokens + override_stop_token_count);
    } else {
      override_opt.emplace();
    }
  }
  return new xgrammar_grammar_matcher{xgrammar::GrammarMatcher(
      compiled_grammar->obj, override_opt, terminate_without_stop_token,
      max_rollback_tokens)};
}
...
bool xgrammar_matcher_fill_next_token_bitmask(xgrammar_grammar_matcher *matcher,
                                              ...
                                              int32_t index) {
  if (!matcher || !bitmask_data || bitmask_count <= 0)
    return false;
...
xgrammar_tokenizer_info *xgrammar_tokenizer_info_create(
  ...
  return new xgrammar_tokenizer_info{
      xgrammar::TokenizerInfo(vec, to_vocab_type(vocab_type), vocab_size_opt,
                              stop_opt, add_prefix_space)};
}
...
xgrammar_tokenizer_info *xgrammar_tokenizer_info_create_from_vocab_and_metadata(
  ...
  auto vec = to_string_vector(encoded_vocab, encoded_vocab_count);
  return new xgrammar_tokenizer_info{
      xgrammar::TokenizerInfo::FromVocabAndMetadata(vec,
                                                    std::string(metadata))};
}
...
xgrammar_tokenizer_info *xgrammar_tokenizer_info_create_from_serialized_json(
  ...
  if (std::holds_alternative<xgrammar::TokenizerInfo>(result)) {
    if (out_error_kind)
      *out_error_kind = XGRAMMAR_ERROR_NONE;
    return new xgrammar_tokenizer_info{
        std::get<xgrammar::TokenizerInfo>(result)};
  }
...
int32_t
xgrammar_tokenizer_info_vocab_size(const xgrammar_tokenizer_info *info) {
...
char *xgrammar_tokenizer_info_detect_metadata_from_hf(const char *backend_str) {
  return copy_string(
      xgrammar::TokenizerInfo::DetectMetadataFromHF(std::string(backend_str)));
}
...
xgrammar_tokenizer_info_decoded_vocab_at(const xgrammar_tokenizer_info *info,
  ...
  if (index < 0 || index >= static_cast<int32_t>(vocab.size()))
    return copy_string("");
...
void xgrammar_set_max_recursion_depth(int32_t depth) {
  xgrammar::SetMaxRecursionDepth(depth);
}
